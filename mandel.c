...
int main(int argc, char *argv[])
{
  ...
  int a,b;
	...
	xmax = 2;
	ymax = 2;
	w = h = 800; //Nombre de ligne et de colonne
	prof = 10000;
  ...
  b=a+(h/p)-1;
	...
	ima = (unsigned char *)malloc((h*w/p)*sizeof(unsigned char));
  unsigned char *imarcv=(unsigned char *)malloc((h*w*sizeof(unsigned char)));
	if (ima == NULL) {
  ...
  /*if (rang==0){
    for (int k = 1; k < p; k++) {
      if(MPI_Recv(&imarcv[k*(h/p)],w*(h/p),MPI_UNSIGNED_CHAR,k,tag,MPI_COMM_WORLD,&status)==MPI_SUCCESS){
        printf("J'ai réussi recv \n");
      }
    }
    sauver_rasterfile("mand_samy.ras", w, h, imarcv);
    //printf("après raster file\n");
  }
  else{
    if(MPI_Send(ima,w*h/p,MPI_UNSIGNED_CHAR,0,tag,MPI_COMM_WORLD)== MPI_SUCCESS) printf("J'ai réussi send \n)");
  }*/
  MPI_Gather(ima,w*(h/p), MPI_CHAR, &imarcv[rang*(h/p)], w*(h/p), MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
  if (rang==0) {
    sauver_rasterfile("mand_samy.ras", w, h, imarcv);
  }
